...
> - **物理层**：在物理介质中进行**比特流**的传输。
> - **数据链路层**：数据以**帧**（frame）的形式传递，通过**MAC地址**进行硬件寻址，提供了数据传输错误校验功能。
> - **网络层**：数据以**数据包**（package）的形式传递，通过IP地址进行逻辑寻址，可实现数据包路由。
> - **传输层**：数据以**段**（fragment ）的形式传输，通过TCP、UDP等协议实现断到端通信。此层指定了所用的通信端口。
> - **会话层**：负责建立、管理和终止表示层实体之间的通信**会话**（session）。SSL/TLS就位于这一层
> - **表示层**：负责对传输的数据进行处理，如数据的格式转换、编码/解码、加密/解密、压缩/解压缩等。
> - **应用层**：为操作系统和网络应用程序提供接口，为用户提供服务。
