...
> &emsp;&emsp;Docker的数据采用层状存储模式，构建镜像时不断向上叠加新的只读层，创建容器时在镜像的基础上添加一个读写层，对容器的所有操作都是在这个读写层上进行的。
>
> &emsp;&emsp;新文件写入时，若该文件不存在，则按需在宿主机上分配空间（allocate-on-demand）；若同名文件位于只读层，则从只读层复制该文件的副本到读写层并隐蔽原文件（copy-on-write）；若要删除只读层文件，则在读写层添加一个删除标记。
>
