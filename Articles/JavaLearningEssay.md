...
|    HashSet    |  "hash"Array  |      true       |   true   |    false     |     Set      |
...
InputStreamReader fir = new InputStreamReader(frs, StandardCharsets.UTF_8);   
fir.read(cbuf);
fir.close();
...
>
> &emsp;&emsp;注解的本质就是一个继承自 Annotation 接口的接口。
...
&emsp;&emsp;JWT令牌由三部分组成，结构为Header.Payload.Signature，具体为：
...
- 荷载（Payload）：由有关实体（通常为用户）和其他数据经Base64编码得到
